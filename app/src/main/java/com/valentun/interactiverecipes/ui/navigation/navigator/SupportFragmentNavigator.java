...
public abstract class SupportFragmentNavigator implements Navigator {
    ...
    private Boolean preventScreenDuplication = true;
    ...
    public void setPreventScreenDuplication(Boolean enableScreenDuplication) {
        this.preventScreenDuplication = enableScreenDuplication;
    }
    ...
    protected void forward(Forward command) {
        // do not let the same screen be opened twice in the row
        String lastScreen = localStackCopy.peekLast();
        if (preventScreenDuplication && lastScreen != null && lastScreen.equals(command.getScreenKey())) {
            return;
        }
